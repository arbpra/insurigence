...
export const quickRiskGuidePrompt: PromptDefinition<QuickRiskGuideOutput> = {
  ...
  instructions: [
    ...
    '- parsedFacts: extract ONLY facts the agent actually stated. Use numbers for revenue/employees/yearsInBusiness/requestedLimit (null if not stated). Use a 2-letter state code, or "" if not stated. priorLosses is true/false only if stated, else null. Do NOT infer or invent values.',
    '',
