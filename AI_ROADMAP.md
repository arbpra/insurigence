...
## Previously-deferred items — NOW BUILT ✅

- **ACORD 130 (Workers' Comp).** `lib/acord/acord130.ts` spec+mapper (payroll/state/class based, not revenue), registered in the ACORD registry. Auto-picked up by the form selector, mapping, validation, review UI, and PDF export. Verified: flags `estimatedAnnualPayroll` (not in GL intake) as expected.
- **Official ACORD template fill.** `lib/acord/templateFill.ts` fills a licensed fillable ACORD PDF (via pdf-lib form fields) when the agency drops one in `acord-templates/<FORMTYPE>.pdf`; field-name mapping in `lib/acord/templateMap.ts`; discovery tool `script/list-acord-fields.ts`. Export route prefers the template, falls back to the generated data PDF when absent. Client action: supply licensed templates + fill the field map.
- **Scanned/image OCR for Document Summary.** `lib/ai/ocr.ts` uses OpenAI vision/file input (no native canvas dep). Upload route now accepts images (png/jpg/webp) and OCRs scanned/image-only PDFs (falls back to OCR when unpdf finds no text), then runs the same deterministic extraction. Verified the OCR call transcribes a PDF correctly.

## Next phase — Coverage Recommendations (in progress)

Rules-first coverage library decides which coverages are recommended + level; AI only explains. Appears in Quick Risk Guide, Formal Evaluation, and an editable Proposal-builder panel. Guardrails: labeled "recommended"/"considerations" (never "required"), soft language, disclaimers everywhere, internal-first, agent can edit/remove/add.

Plan (~2 hrs/day):
- **Day 1 — Coverage library + engine (deterministic).** ✅ `lib/coverage/` — `types`, `constants` (both disclaimers), `library` (9 coverages: GL, Garagekeepers, WC, Commercial Auto, Property, Inland Marine, Cyber, EPLI, Umbrella with editable rules), `engine` (`computeCoverageRecommendations`). Verified: matches the brief's auto-detailing example (GL/Garagekeepers strongly, WC/Auto recommended, Inland Marine/Cyber/EPLI/Umbrella consider); solo consultant → most not-typically-needed.
- **Day 2 — schema + exposure parsing + AI explanation layer.** ✅ Added exposure flags (hasBusinessVehicles, customerPropertyInCare, mobileEquipment, storesCustomerData, ownsOrLeasesProperty, higherLimitsDesired) to `ParsedRiskFactsSchema` + tri-state boolean coercion; Quick Risk Guide prompt v1.2.0 extracts them. New `CoverageExplanationRefinementSchema` + `coverageRecommendationExplanationPrompt` (AI refines wording only) + `applyAiExplanations()` merge helper (levels/triggers stay deterministic). Verified live: AI tailored all 9 explanations, levels 100% preserved.
- **Day 3 — Quick Risk Guide placement.** ✅ quick-risk-guide route now builds exposure facts from parsedFacts → `computeCoverageRecommendations` → AI refines explanations → merges → saves a `coverage_recommendations` AiRun → returns `coverageRecommendations` + display disclaimer. `CoverageRecommendationsCard` in the widget: level badges (Strongly→Not-typically-needed), agent explanation, reason, triggering exposures, cross-sell flag, disclaimer, "Guidance · agent review required." Verified live: free text → parsed exposures → correct recommendations matching the brief.
- **Day 4 — Formal Evaluation engine placement.** ✅ `lib/coverage/factsFromIntake.ts` deterministically builds exposure facts from stored intake (explicit fields + conservative keyword inference from the description — no AI). The `evaluate` route now computes coverage recommendations alongside carrier fits, persists them to a `coverage_recommendations` AiRun (model `rules-engine`), and returns them + disclaimer. Verified: flat + dotted intake shapes; keyword inference catches customer-property-in-care, mobile equipment, online booking. Proposal-page display + editing is Day 5.
- **Day 5 — editable Proposal-builder panel.** ✅ New `CoverageRecommendationSet` model (one editable set per lead, `@unique` leadId, status DRAFT/REVIEWED + review trail; stored separately for audit). `app/api/ai/coverage-recommendations/[leadId]` GET (saved set + library names + disclaimer) / POST (seed from intake exposures + AI-refined text, upsert) / PATCH (save agent edits + approve/mark-draft). `CoverageRecommendationsPanel.tsx` on the proposal page: agent can change level, edit explanation, toggle cross-sell, remove, add coverage (from library), Save / Approve. Disclaimer + review status always shown. Guidance only; agent-review layer before proposal use.
- **Day 6 — super-admin coverage-rules editor.** ✅ New `CoverageRuleConfig` model (global). `lib/coverage/config.ts` (`loadCoverageConfigs` + `applyCoverageConfigs` — filter disabled, apply explanation overrides, append custom coverages, re-sort) merged into all 3 producing routes (quick-risk-guide, evaluate, coverage-recommendations). `app/api/super-admin/coverage-rules` GET/PUT + `app/(main)/super-admin/coverage-rules` page: enable/disable each coverage, override explanation text, add custom coverages (name/level/reason). Nav link added. Verified live: disable excludes, explanation override wins, custom coverage appears. Engine's level logic stays in code.

**Coverage Recommendations feature COMPLETE ✅** — all 3 placements (Quick Risk Guide, Formal Evaluation, editable Proposal panel) + super-admin rule editor.
